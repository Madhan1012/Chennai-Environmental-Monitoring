...
def calculate_risk_dashboard():
    try:
        ...
        SELECT 
            ...
            AVG(CASE WHEN m.pollutant = 'windspeed' THEN m.value END) as avg_wind
        ...
        def get_risk_meta(row):
            ...
            if score > 100:
                return {'level': 'High', 'color': '#d73027', 'advice': 'Wear N95 masks; limit outdoor exercise.'}
            elif score > 50:
                return {'level': 'Moderate', 'color': '#fee08b', 'advice': 'Sensitive groups should reduce heavy exertion.'}
            else:
                return {'level': 'Low', 'color': '#1a9850', 'advice': 'Air quality is good. Enjoy outdoor activities.'}
        ...
        for feature in choropleth.geojson.data['features']:
            ...
            if name in data_dict:
                info = data_dict[name]
                feature['properties']['Risk'] = f"{info['level']} ({info['risk_score']:.1f}%)"
                feature['properties']['PM25'] = f"{info['avg_pm25']:.1f} µg/m³"
                feature['properties']['Wind'] = f"{info['avg_wind']:.1f} m/s" if info['avg_wind'] else "No Data"
        ...
        folium.GeoJsonTooltip(
            ...
            aliases=['Zone:', 'Risk Level:', 'Avg PM2.5:', 'Avg Wind:', 'Recommendation:'],
